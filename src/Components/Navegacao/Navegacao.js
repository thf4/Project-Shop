/* eslint-disable no-unused-vars */
import "./Exemplo.css";

const navegation = [
  {
    img:
      "data:image/jpeg;base64,",
    alt: "Area De Beleza",
    links: "#",
    nameSection: "Moda Feminina",
  },
  {
    img:
      "https://encontresuafranquia.com.br/wp-content/uploads/2020/10/franquia-de-moda-masculina-730x458.jpg",
    alt: "Moda Masculina",
    links: "#",
    nameSection: "Moda Masculina",
  },
  {
    img:
      "https://i.pinimg.com/originals/db/84/93/db84933b6bd121c2817b184c3c9e85e2.jpg",
    alt: "Moda Infantil",
    links: "#",
    nameSection: "Moda Infantil",
  },
  {
    img:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6bNGI8YFg22S1EwA4ux8aDVgF2KcIkRIx5g&usqp=CAU",
    alt: "Conforto",
    links: "#",
    nameSection: "Conforto",
  },
  {
    img: "https://midias.agazeta.com.br/2020/04/23/dicas-233769-article.jpg",
    alt: "Beleza",
    links: "#",
    nameSection: "Beleza",
  },
  {
    img:
      "https://vestrio.vteximg.com.br/arquivos/ids/171352-475-475/Conjunto-Cropped-com-Bojo-em-Renda-Preto.jpg?v=637503890205730000",
    alt: "Lengerie",
    links: "#",
    nameSection: "Lengerie",
  },
  {
    img:
      "https://agendasette.com.br/wp-content/uploads/2018/08/FOTO-2-800x445.jpg",
    alt: "Calçados",
    links: "#",
    nameSection: "Calçados",
  },
];

function Navegacao() {
  return (
    <div>
      <h2>Navegue por departamento</h2>
      <section className="list">
        {navegation.map((foto) => {
          const { img, alt, links, nameSection } = foto;
          return <Foto key={img} foto={foto}></Foto>;
        })}
      </section>
    </div>
  );
}

const Foto = (props) => {
  const { img, alt, links, nameSection } = props.foto;

  return (
    <article className="foto-banner">
      <a className="link" href={links}>
        <img className="foto" src={img} alt={alt} /> {nameSection}
      </a>
    </article>
  );
};

export default Navegacao;
